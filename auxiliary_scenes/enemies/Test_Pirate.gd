...
var bulletB = preload("res://bullets/Laser_0.tscn")
...
func process_idle():
	...
	if idle_counter >= 20:
		...
		State.game_ui.set_attack_name("bomb")
		State.bullets_manager.clear_bullets()
	pass

func process_bomb():
	if bomb_counter > 120:
		var n = (bomb_count + 1) * 10
		for i in range(n):
			if i % 6 != 0:
				var b = [position, 17, [[0, i * (360 / n), 3, 0, false]]]
				State.bullets_manager.add_bullet(b)
				#State.bullets_manager.add_simple_laser(position, 0, (i * 12) + (sin(bomb_counter / 100.0) * 15), 3, 120)
	bomb_counter += 1
	if bomb_counter >= 400:
		bomb_counter = 0
		bomb_count += 1
...
func process_megaton():
	if megaton_counter < 256 and megaton_counter > 64 and megaton_counter % 8 == 0:
		...
		var b1a = []
		for j in range(8):
			b1a.push_back([0, [[0, (j * 45) - angle, 4, 0, false]]])
